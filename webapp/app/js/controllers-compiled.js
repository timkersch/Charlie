...
var charlieController = angular.module('charlieController', ['ngRoute', 'charlieService', 'ngMaterial']);

charlieController.controller('mainController', ['$scope', '$routeParams', '$location', '$mdToast', 'charlieProxy', '$mdSidenav', function ($scope, $routeParams, $location, $mdToast, charlieProxy, $mdSidenav) {
    $scope.user = {};
    $scope.url = "";
    ...
    var showActionToast = function showActionToast(quiz) {
        var toast = $mdToast.simple().textContent('You have been invited to ' + quiz.name).action('ACCEPT').highlightAction(true).hideDelay(10 * 1000);
...
charlieController.controller('lobbyController', ['$scope', '$location', 'charlieProxy', function ($scope, $location, charlieProxy) {
    ...
    $scope.startQuiz = function () {
        ...
        if (charlieProxy.isQuizOwner()) charlieProxy.nextQuestion(function (data) {});
    };
...
charlieController.controller('homeController', ['$scope', '$location', 'charlieProxy', function ($scope, $location, charlieProxy) {
...
charlieController.controller('questionController', ['$scope', '$location', '$interval', 'charlieProxy', '$document', '$timeout', function ($scope, $location, $interval, charlieProxy, $document, $timeout) {
    ...
    var init = function init() {
        ...
        charlieProxy.getCurrentQuestion(function (question) {
            if (question.artists) {
                ...
                if (charlieProxy.isQuizOwner()) play(question.track_url);
                startInterval();
            }
        });
    };
    ...
    charlieProxy.listenTo("userPointsUpdate", function (player) {
        ...
        if (!found) $scope.players.push(player);
    });
    ...
    var startInterval = function startInterval() {
        ...
        intervalPromise = $interval(function () {
            if ($scope.timeLeft > 0) $scope.timeLeft--;
...
charlieController.controller('scoreboardController', ['$scope', '$document', '$location', 'charlieProxy', function ($scope, $document, $location, charlieProxy) {
...
charlieController.controller('profileController', ['$scope', 'charlieProxy', function ($scope, charlieProxy) {
...
charlieController.controller('createController', ['$scope', '$location', 'charlieProxy', function ($scope, $location, charlieProxy) {
